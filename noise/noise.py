...
def ps_noise(dof):
    ...
    noise = np.loadtxt(os.path.join(direc,'ps_6dof.txt'))
...
def _ipgas_lvdt_noise(dof=None):
    ...
    '''
    direc = os.path.dirname((__file__))
    noise = np.loadtxt(os.path.join(direc,'ipgas_lvdt.dat'))
...
def _bf_lvdt_noise(dof):
    ...
    '''
    direc = os.path.dirname((__file__))
    noise = np.loadtxt(os.path.join(direc,'bf_lvdt.dat'),delimiter=',')
    freq, value = noise[:,0],noise[:,1]
...
def seismicnoise(unit='m',axis='hor',percentile='90'):
    direc = os.path.dirname((__file__))
    fname_fmt = direc+'/seismicnoise/{0}_{1}_DISP.txt'
    if axis=='hor':
