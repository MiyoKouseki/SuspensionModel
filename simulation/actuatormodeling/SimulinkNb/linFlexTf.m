...
%display = varargin{4};    % COMMENTED OUT BY YM ON 10Apr2018
...
%if display    % COMMENTED OUT BY YM ON 10Apr2018
disp([num2str(numel(flexTfBlocks)) ' FlexTf blocks found in model ' strtrim(evalc('disp(mdl)'))]);
%end    % COMMENTED OUT BY YM ON 10Apr2018

if numel(flexTfBlocks) < 1
    warning('linFlexTf:NoFlexTfblocks','No FlexTf blocks found: standard linearization will be performed');
