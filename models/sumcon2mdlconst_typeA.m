...
matPara = {
    {184e9, 0.32, 184e9/2/(1+0.32), 8.0e3, 2.0e9, 1.0e3},; %Maraging Steel 
    {200e9, 0.28, 200e9/2/(1+0.28), 7.8e3, 2.0e9, 1.0e4},; %C-70 Steel
    {411e9, 0.28, 411e9/2/(1+0.28),19.3e3, 2.0e9, 1.0e4},; %Tungsten
    {392e9, 0.23, 392e9/2/(1+0.28),3.98e3, 2.0e9, 0.5e4},; %Sapphire (300K)
    {127e9, 0.35, 127e9/2/(1+0.28),8.36e3, 2.0e9, 0.2e6},; %Copper Beryllium(300K)
    {398e9, 0.22, 398e9/2/(1+0.28),3.98e3, 2.0e9, 0.5e7},; %Sapphire (20K)
    {134e9, 0.35, 134e9/2/(1+0.28), 8,4e3, 2.0e9, 2.0e5},; %Copper Berylium (20K)
    {200e9, 0.32, 200e9/2/(1+0.28), 8.0e3, 2.0e9, 1.0e3} % Maraging Steel (20K)
%     {345e9, 0.30, 345e9/2/(1+0.30), 4.0e3, 2.0e9, 0.2e6},; %Sapphire
...
%sumconFileName = 'D:\KAGRA SVN\VIS\sumcon\save\TypeA\ETMY191113.m';
sumconFileName = 'D:\KAGRA SVN\VIS\sumcon\save\TypeA\TypeA180429_20K.m';
susmodelname = 'ITMY';
...
tmp_param = strrep(tmp_param,char(13),'');
...
% at PF
% for i =1:length(sus.w.n)
%     if strcmp(cell2mat(sus.w.lb(i)),'F1')
%         sus.w.s(i) = {[cell2mat(sus.w.s(i))*4]};
%     end
% end
...
%                     4,...               % number of wires
%                     {'spring'},...                     % OPTION
%                     groundname,...                   % upper body
%                     'PF',...                   % lower body
%                     [...                       % GEOMETRIC PARAMETER
%                     0,...              % vertical position of upper SP
%                      0.1,...              % vertical position of lower SP
%                      0.1,...              % transversal position of upper SP
%                      0.1,...              % longitudinal position of upper SP
%                     ],...                      %
%                     1.0,...                 % length
%                     0.003,...   % diameter & neck diameter
%                     100,...               % total load
%                     E,...               % Young's modulus
%                     G,...               % shear modulus
%                     3.0,...                % Q factor
%                     {'spring',...              % SPRING
%                     0.01,...             % resonant frequency
%                     GASinfo{GASproperty}{3},...             % saturation due to CoP
%                     GASinfo{GASproperty}{4}}...              % spring Q
%                     );
...
%                     [...                       % GEOMETRIC PARAMETER
%                     0,...              % vertical position of upper SP
%                      0.03,...              % vertical position of lower SP
%                      0.03,...              % transversal position of upper SP
%                      0.03,...              % longitudinal position of upper SP
%                     ],...                      %
%                     1.0,...                 % length
%                     0.003,...   % diameter & neck diameter
%                     10,...               % total load
